...
public class GrantByResourceOwnerPasswordCredentialTest {
    ...
    public void getJwtTokenByClientCredentialForUser() throws JsonParseException, JsonMappingException, IOException {
    	...
    	ResponseEntity<String> response = new TestRestTemplate(userName, password).postForEntity("http://localhost:" + port + "/oauth/token?grant_type=password&username='"+userName+"'&password='"+password+"'", null, String.class);
    ...
    public void getJwtTokenByClientCredentialForAdmin() throws JsonParseException, JsonMappingException, IOException {
    	...
    	ResponseEntity<String> response = new TestRestTemplate(userName, password).postForEntity("http://localhost:" + port + "/oauth/token?grant_type=password&username=admin&password=password", null, String.class);
        String responseText = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        HashMap jwtMap = new ObjectMapper().readValue(responseText, HashMap.class);
    ...
    public void accessProtectedResourceByJwtTokenForUser() throws JsonParseException, JsonMappingException, IOException {
        ...
        response = new TestRestTemplate(userName, password).postForEntity("http://localhost:" + port + "/oauth/token?grant_type=password&username=user&password=password", null, String.class);
        String responseText = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        HashMap jwtMap = new ObjectMapper().readValue(responseText, HashMap.class);
        String accessToken = (String) jwtMap.get("access_token");
    ...
    public void accessProtectedResourceByJwtTokenForAdmin() throws JsonParseException, JsonMappingException, IOException {
        ...
        response = new TestRestTemplate(userName, password).postForEntity("http://localhost:" + port + "/oauth/token?grant_type=password&username=admin&password=password", null, String.class);
        String responseText = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        HashMap jwtMap = new ObjectMapper().readValue(responseText, HashMap.class);
        String accessToken = (String) jwtMap.get("access_token");
